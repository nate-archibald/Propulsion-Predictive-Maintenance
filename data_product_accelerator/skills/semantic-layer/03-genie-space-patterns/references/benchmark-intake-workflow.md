...
After initial deployment, benchmark questions serve a second purpose: detecting regressions when instructions, assets, or metric views change.

### When to Run Regression Tests

| Trigger | What Changed | Risk Level |
|---------|-------------|------------|
| Instruction append (optimization loop) | Routing rules updated | 🔴 High — new rules may conflict with existing |
| Metric View YAML edit | Dimensions or measures modified | 🔴 High — silent dimension loss possible |
| Trusted asset added/removed | Data source set changed | 🟡 Medium — routing may shift |
| SQL Expression update | KPI/filter definitions changed | 🟡 Medium — matching may break |
| No changes (weekly cadence) | Configuration drift via UI | 🟢 Low — but catches manual edits |

### Regression Test Workflow

```
1. Load pinned benchmark suite (fixed date ranges, no CURRENT_DATE)
2. For each benchmark question:
   a. Submit to Genie via Conversation API
   b. Compare returned SQL structure to expected SQL
   c. Compare row count to expected range
   d. Record pass/fail with diff details
3. Generate regression report:
   - Overall pass rate (target: 100%)
   - Per-question results with diffs
   - Regressions flagged for investigation
4. If pass rate < threshold:
   - Block promotion (CI/CD gate)
   - Flag regressions for manual review
```

### Temporal Expression Policy

Benchmark SQL for regression testing MUST use pinned date ranges:

```sql
-- ❌ Non-deterministic — different results each day
WHERE transaction_date >= DATE_TRUNC('month', CURRENT_DATE)

-- ✅ Pinned — same results every run
WHERE transaction_date BETWEEN DATE '2026-01-01' AND DATE '2026-03-31'
```

For benchmarks that must reference "recent" data, use a configurable anchor date:

```python
BENCHMARK_ANCHOR_DATE = os.getenv("BENCHMARK_ANCHOR_DATE", "2026-03-31")
```

### Comparing SQL Structure (Not Exact Match)

Genie may generate SQL that differs syntactically but is semantically equivalent. Compare:

| Compare | Method | Tolerance |
|---------|--------|-----------|
| Table references | Extract FROM/JOIN targets | Exact match required |
| Aggregation functions | Extract SELECT aggregations | Same function, same column |
| WHERE conditions | Extract filter predicates | Same columns and operators |
| Row count | Compare result set size | Within ±10% of expected |
| Column names | Compare result schema | Same columns returned |

Use `assets/templates/genie-space-regression-test.py` as a starting template.
