...
## Dashboard, Metric View & Genie Space Naming
...
| Domain | Report Type | Result |
|--------|-------------|--------|
| `Sales` | `Revenue Overview` | `Sales - Revenue Overview` |
| `Billing` | `Cost Attribution` | `Billing - Cost Attribution` |
| `Hospitality` | `Booking Trends` | `Hospitality - Booking Trends` |
| `Platform` | `Data Quality Summary` | `Platform - Data Quality Summary` |
...
|--------|--------|--------|
| `billing` | `cost_summary` | `mv_billing_cost_summary` |
| `sales` | `revenue` | `mv_sales_revenue` |
| `hospitality` | `booking_occupancy` | `mv_hospitality_booking_occupancy` |

### Genie Spaces

```
{Domain} {Description} Space
```

| Domain | Description | Result |
|--------|-------------|--------|
| `Hospitality` | `Booking Analytics` | `Hospitality Booking Analytics Space` |
| `Sales` | `Revenue Insights` | `Sales Revenue Insights Space` |
| `Billing` | `Cost Explorer` | `Billing Cost Explorer Space` |

---

## Canonical Domain Names

Use these canonical domain names consistently across all assets (schemas, jobs, pipelines, dashboards, Genie Spaces). Pick the canonical name and never use the alternatives.

| Canonical Domain | Use When | Do NOT Use (Aliases) |
|------------------|----------|----------------------|
| `hospitality` | Hotels, bookings, guests, properties, reservations | `booking`, `hotel`, `travel`, `lodging` |
| `sales` | Revenue, orders, customers, transactions | `revenue`, `commerce`, `orders` |
| `billing` | Cost, usage, invoicing, DBU consumption | `finance`, `cost`, `pricing` |
| `marketing` | Campaigns, leads, attribution, engagement | `campaigns`, `demand_gen` |
| `operations` | Logistics, supply chain, fulfillment | `ops`, `logistics`, `fulfillment` |
| `platform` | Infrastructure, data quality, system monitoring | `infra`, `system`, `engineering` |
| `analytics` | Cross-domain reporting, executive dashboards | `reporting`, `bi` |
| `hr` | Employees, payroll, workforce | `people`, `workforce`, `talent` |

**Derivation rule:** When the schema CSV or catalog name implies a domain, map it to the canonical name above. If a project spans multiple domains, use the primary domain for shared assets and domain-specific names for scoped assets.
