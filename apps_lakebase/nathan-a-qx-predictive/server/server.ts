    ...
    appkit.server.extend((app) => {
      ...
      app.get("/api/critical-spares-debug", async (req: Request, res: Response) => {
        ...
        try {
          ...
          // Show which would qualify as "spares" with current logic
          const sparesWithCurrentLogic = latestPerSn.filter((r: any) => 
            ['r/i', 'r/i-nla', 'r/i-nlk'].includes(String(r.transaction_type_clean || ''))
          );

          console.log(`[DEBUG] Latest per SN: ${latestPerSn.length}, Spares: ${sparesWithCurrentLogic.length}`);
          ...
          res.json({
            ...
            summary: {
              ...
              spares_found: sparesWithCurrentLogic.length,
            ...
            spares_with_current_logic: sparesWithCurrentLogic,
          });
        } catch (err) {
