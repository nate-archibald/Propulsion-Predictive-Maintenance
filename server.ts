...
async function handleGenieQuery(
  ...
  // 2. Poll the message until status leaves IN_PROGRESS / EXECUTING_QUERY
  ...
  for (const att of attachments) {
    ...
    if (att?.text?.content) {
      reply = att.text.content;
    }
    // Generated SQL
    if (att?.query?.query) {
      steps.push(`SQL: ${att.query.query}`);
    }
    // Query description as fallback answer
    if (att?.query?.description && !reply) {
      reply = att.query.description;
    }
  }

  // Fallback if no text attachment found
  if (!reply && msg.query_result?.row_count !== undefined) {
    reply = `Query completed — returned ${msg.query_result.row_count} rows.`;
  }
